...
#     ifndef NDEBUG
	...
	{ outfmt(logchn,gtxecho,"\n      [%s]: scaling not required;",
		 orig_sys->nme) ; }
	outfmt(logchn,gtxecho," %g <= |a<ij>| <= %g, metric = %g.",
...
/*
  Do we need to scale? If the client provided us with scaling vectors, store them
  with the local copy. A full attach isn't required, as the local copy will be
  static through the dylp run and any hot starts.
*/
  if (scale == TRUE)
...
#     ifndef NDEBUG
	...
	outfmt(logchn,gtxecho,"\n      [%s]: after geometric scaling",
	       local_sys->nme) ;
	outfmt(logchn,gtxecho," %g <= |a<ij>| <= %g, geom = %g.",
