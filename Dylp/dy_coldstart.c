...
# ifndef NDEBUG
  ...
  { outfmt(logchn,gtxecho,"\n    found %d equalities, %d inequalities",
	   eqcnt,ineqcnt) ;
    if (eqcnt+ineqcnt < m)
    { outfmt(logchn,gtxecho,", discarded %d empty constraints",
	     (m-eqcnt-ineqcnt)) ; }
...
# ifndef NDEBUG
  ...
  { outfmt(logchn,gtxecho,
    ...
    { outfmt(logchn,gtxecho,", %c %.4f %.4f %c",
...
# ifndef NDEBUG
  ...
  { outfmt(logchn,gtxecho,
    ...
    if (nbfxcnt > 0) outfmt(logchn,gtxecho," (%d fixed)",nbfxcnt) ;
    outfmt(logchn,gtxecho," remain inactive in system %s.",orig_sys->nme) ;
...
#     ifndef NDEBUG
	...
	outfmt(logchn,gtxecho," to cover %s (%d),",
	       consys_nme(dy_sys,'c',i,FALSE,NULL),i) ;
	outfmt(logchn,gtxecho," |a<%d,%d>/max(a<*,%d>)| = %g.",
	       i,j,j,ratio) ; }
#     endif
      if (covered == m) break ; }
#   ifndef NDEBUG
    ...
    { if (pkndx < 0)
      { outfmt(logchn,gtxecho,
	       ...
	       consys_nme(dy_sys,'v',j,FALSE,NULL),j) ;
	outfmt(logchn,gtxecho," a<%d,%d> = %g, estpiv<%d> = %g, ratio %g.",
...
#   ifndef NDEBUG
	...
	outfmt(logchn,gtxecho," with value %g.",dy_x[vndx]) ;
      else
	outchr(logchn,gtxecho,'.') ; }
#   endif
  }
...
*/
  ...
  { outfmt(logchn,gtxecho,
    ...
    { vndx = dy_basis[cndx] ;
      ...
      outfmt(logchn,gtxecho,"  (%4d) %-13s %-7s %7g",vndx,
  ...
  { outfmt(logchn,gtxecho,"\n\tinitial objective %g",dy_lp->z) ;
    ...
    outfmt(logchn,gtxecho,", target simplex %s.",
