...
/*
  ...
  we should have retained data structures. Iteration limit is (sort of) a
  special case -- this arises frequently when a B&C code is using dylp for
  strong branching; the iteration limit will be deliberately inadequate.

  Admittedly these checks are not foolproof (the status flag and return code
  are accessible to the client, and we could be retaining structures based on
  a call from some other client), but one would like to hope the client knows
  what it's doing and is not arbitrarily interleaving calls from different
  objects. If we're paranoid, we'll check for consistency.
*/
# ifdef PARANOIA
